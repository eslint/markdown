 ...
 *
 * The source code of a non-configuration-comment line in the linted JS is
 * a suffix of the corresponding line in the Markdown code block. There are no
 ...
 * @returns {number[]} A flattened list of 2-tuples, where the even-indexed
 *     elements are range offsets in the linted JS used as a lookup key, and the
 *     next odd-indexed element is the offset delta from linted JS to Markdown.
 *     Add this value to a range from the linted JS to get the equivalent
 *     location in the Markdown source.
 */
        ...
        if (message.fix) {
            adjustedFix.fix = {
                range: message.fix.range.map(function(range) {
                    ...
                    // matching range.
                    var i = 0;
                    while (i < block.rangeMap.length && block.rangeMap[i] < range) {
                        i += 2;
                    }

                    // Apply the mapping delta for this range.
                    return range + block.rangeMap[i - 1];
                }),
