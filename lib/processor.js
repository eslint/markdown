...
function adjustBlock(block) {
    ...
    var blockEnd = block.position.end.line;
    var blockLines = blockEnd - blockStart;
    ...
    return function adjustMessage(message) {
        ...
        var adjustedFix = {};
        if (message.fix) {
            var blockOffset = block.position.end.offset - block.position.end.column + (block.position.start.column - 1);
            var codeLength = block.value.length;
            var commentsLength = block.comments.reduce(function(acc, curr) {
                return acc + curr.length + 1;
            }, 0);
            var indentLength = (block.position.indent[lineInCode - 1] - 1) * (blockLines - lineInCode);
            var offset = blockOffset - codeLength - commentsLength - indentLength;
            adjustedFix.fix = {
